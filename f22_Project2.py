...
.
    ...
    with open('html_files/listing_' + listing_id + '.html', 'r') as f:
        ...
        #final tuple
        # print (policy, place_type, num_bedroom)
...
def check_policy_numbers(data):
    ...
    """
    incorrect_policies = []
    regex = '(20[0-9]{2}-00[0-9]{4}STR)|(STR-000[0-9]{4})'

    for listing in data:
        policy_num = listing[3]
        #ignore pendning or exempt
        if "pending" not in policy_num.lower() and "exempt" not in policy_num.lower():
            if re.search(regex, policy_num) == None:
                incorrect_policies.append(listing[2])
    
    # print(listing)
    # print(incorrect_policies)
    return incorrect_policies
